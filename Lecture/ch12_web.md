...
```
[ 用戶端 (Browser) ] ---- 發送 HTTP 請求 (Request) ---> [ 伺服器端 (Python/Flask) ]
[ 用戶端 (Browser) ] <--- 回傳 HTTP 回應 (Response) --- [ 伺服器端 (Python/Flask) ]
```
...
```python
from flask import Flask, request, render_template_string, redirect, url_score
